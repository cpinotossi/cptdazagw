...
resource "azurerm_web_application_firewall_policy" "with_exclusion" {
  ...
  # Custom Rule: ALLOW /admin/users path (bypasses 941170)
  custom_rules {
...
resource "azurerm_public_ip" "appgw1" {
  name                = "${local.resource_prefix}-appgw1-pip-${random_id.suffix.hex}"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_public_ip" "appgw2" {
  name                = "${local.resource_prefix}-appgw2-pip-${random_id.suffix.hex}"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
  allocation_method   = "Static"
  sku                 = "Standard"
...
resource "azurerm_application_gateway" "standard" {
  name                = "${local.resource_prefix}-appgw-standard-${random_id.suffix.hex}"
  resource_group_name = azurerm_resource_group.main.name
  location            = azurerm_resource_group.main.location
  tags                = azurerm_resource_group.main.tags
...
resource "azurerm_application_gateway" "with_exclusion" {
  name                = "${local.resource_prefix}-appgw-exclusion-${random_id.suffix.hex}"
  resource_group_name = azurerm_resource_group.main.name
  location            = azurerm_resource_group.main.location
  tags                = azurerm_resource_group.main.tags
